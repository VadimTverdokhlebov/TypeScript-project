...
export default class OrderController {
  static async addOrder(req: Request, res: Response, next: NextFunction) {
    try {
      const userId = (req as unknown as IOrderPaginationRequest).user.id;
  ...
  static async getOrders(req: Request, res: Response, next: NextFunction) {
    try {
      const userId = (req as unknown as IOrderPaginationRequest).user.id;
      const limit = Number((req as unknown as IOrderPaginationRequest).query.limit || 1);
      const page = Number((req as unknown as IOrderPaginationRequest).query.page || 1);
