...
import { IProductsFromClient, IOrder } from '../entities/order';
import { IProduct } from '../entities/product';
import { IAdditive } from '../entities/additive';
...
export function getFilledOrder(
  ...
  for (const product of products) {
    const currentProduct = productsOrder
      .find((elem: IProduct) => elem.id === product.id) as IProduct;
    ...
    for (const additiveId of product.additives) {
      const currentAdditive = additivesOrder
        .find((elem: IAdditive) => elem.id === additiveId) as IAdditive;
